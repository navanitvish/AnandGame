...
import {
  ...
  RefreshCw, Clock, Shield, Layers, Calendar, DollarSign,
    ...
    <div className="fixed inset-0 bg-black/40 z-40 flex items-center justify-center backdrop-blur-sm">
      <div className="bg-white rounded-2xl p-6 shadow-2xl w-80">
        <div className="text-center mb-4">
          ...
          <p className="text-sm text-gray-500">Delete <span className="font-semibold">"{name}"</span>? This cannot be undone.</p>
        </div>
...
function GroundFormModal({ show, editGround, venues, sports, academyOptions, onClose, onSaved, toast }) {
  ...
  const DropDown = ({ label, k, options, ph = 'Select…', required = false }) => (
    <div>
      <label className={labelCls}>{label}{required && ' *'}</label>
      <select value={form[k]}
        onChange={e => { set(k, e.target.value); setErrors(er => ({ ...er, [k]: '' })) }}
  ...
  const SelectField = ({ label, k, opts, ph = 'Select…', required = false }) => (
    <div>
      <label className={labelCls}>{label}{required && ' *'}</label>
      <select value={form[k]}
        onChange={e => { set(k, e.target.value); setErrors(er => ({ ...er, [k]: '' })) }}
      ...
      <div className="bg-white rounded-2xl shadow-2xl w-full max-w-2xl max-h-[92vh] overflow-y-auto">
        ...
        <div className="p-6 space-y-6">
          ...
          <div>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
              <Field label="Ground Name"  k="name"  ph="e.g. Cricket Ground 1" required />
              <SelectField label="Type"   k="type"  opts={GROUND_TYPES} ph="Select type" required />
              <div className="sm:col-span-2">
          ...
          <div>
            ...
            <div className="grid grid-cols-2 sm:grid-cols-3 gap-3">
              <Field label="Opening Time" k="openingTime" type="time" required />
              <Field label="Closing Time" k="closingTime" type="time" required />
              <div>
          ...
          <div>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
              <SelectField label="Status" k="status" opts={STATUS_OPTS} ph="Select status" />
              <div>
          ...
          <BannerUpload
            ...
            existingBanners={editGround?.banners}
          />
